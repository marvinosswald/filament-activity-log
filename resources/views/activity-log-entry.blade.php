    ...
    <ul role="list" class="space-y-6"
        ...
        @foreach($getState()?->sortBy([['id', 'desc'], ['created_at', 'desc']]) ?? [] as $activity)
            <li class="relative flex gap-x-4">
                ...
                @endif
                <div class="relative flex h-6 w-6 flex-none items-center justify-center bg-white dark:bg-gray-900">
                    @switch($activity->event)
                        ...
                        @default
                            <div class="relative flex h-6 w-6 flex-none items-center justify-center ">
                                <div
                                    class="h-1.5 w-1.5 rounded-full bg-gray-100 dark:bg-gray-400 ring-1 ring-gray-300 dark:ring-gray-500"></div>
                            </div>
                            @break
                    @endswitch
                </div>
                <div class="flex-grow flex flex-row">
                    <div class="flex flex-col">
                        <p class="flex-auto py-0.5 text-xs leading-5 text-gray-500">
                            <span
                                class="font-medium dark:text-gray-100 text-gray-900">{{$getCauser($activity)}}</span> {{$activity->event}} {{$getSubject($activity)}}
                            .
                        </p>
                        @if($activity->event === 'updated')
                            ...
                            @endforeach
                        @endif
                        @if($getDescription($activity))
                            <div class="border-l-2 border-gray-500 mt-2">
                                <blockquote
                                    class="pl-2 text-xs font-semibold leading-8 tracking-tight text-gray-900 dark:text-gray-100">
                ...
                <time datetime="{{$activity->created_at}}"
                      class="flex-none py-0.5 text-xs leading-5 text-gray-500">{{$activity->created_at?->diffForHumans()}}</time>
            </li>
        @endforeach
    </ul>
